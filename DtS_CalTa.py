...
res['F1'] = 'Protection thickness, δ, mm'
...
if Task == 1:
    ...
    for e in range(len(E)):
        ...
        res.cell(row=e + 2, column=7).value = ''
        b = 0.
        mud = np.zeros(len(dt))
        ...
        for i in range(len(dt)):
            ...
            res.cell(row=e + 2, column=7).value += ' | ' + str(mud[i])
            print('μd[', i, '] =', mud[i])
            b += (mud[i] * d[i]) / 10
    ...
    aH = a/H
    RH = R/H
    res.cell(row=2, column=10).value = aH
    res.cell(row=2, column=12).value = RH
    for e in range(len(E)):
        res.cell(row=e + 2, column=1).value = E[e]
        res.cell(row=e + 2, column=2).value = q[e]
        res.cell(row=e + 2, column=3).value = float(ApproxLin(tab_En, tab_Ga, E[e]) * q[e])
        ...
        res.cell(row=e + 2, column=8).value = musH
        ...
        if f(musH) > tab_musH[-2] or f(musH) < 0:
            ...
            Z = float(f(musH))
        res.cell(row=e + 2, column=13).value = Z
        V = H * 2 * np.pi * R ** 2
